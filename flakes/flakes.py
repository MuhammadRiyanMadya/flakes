...
class standard(Flakes):
    ...
    def narrowGain(self, SP, PV, KLIN, KGAP, GAPLO, GAPHI):
...
class disturbance(Flakes):
    ...
    def numtxt(self,txt):
        num = ''.join((ch if ch in '0123456789,' else ' ') for ch in txt)
        num = num.replace(',','.') #.split()
        return num
    
    def modelDFile(self,path):
        try:
            lcnt = 0
            f = open(path, "r", encoding="utf-8")
            d = self.numtxt(f.readline())
            if d != '':
                self.k = np.append(self.k,float(d))
            while d != '':
                lcnt += 1
                d = self.numtxt(f.readline())
                if d != '':
        ...
        return self.k, self.klen
        
    def generatorD(self, myD):
        i = 0
        while 1:
            yield myD[i], i
            i +=1
...
##d = disturbance()
##
##j,i = d.modelDFile(r'C:\Users\ssv\Documents\MRM\Flakes\kdtrb.csv')
##
##caller = d.generatorD(j)
##
##while 1:
##    i = 0
##    while i < 1:
...
##        print(D,' ',k)
##    
    def feedD(self,modelD,delayHolder,flipIndex, didx,delay = 0, defaultKp = 1):
        delayHolder += 1
        if (delayHolder - flipIndex) >= delay:
            self.valueD = modelD[didx]
            didx += 1
            if didx == (len(modelD)-1):
                didx = 0
            delayHolder = 0
        else:
            self.valueD = defaultKp
        return self.valueD
