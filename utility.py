...
##        ax1.yaxis.set_major_locator(MultipleLocator(0.025))
    ...
    def largeExt(self,path, gradeKey, FileErrorFlag = False):
        for files in os.listdir(path):
            try:
                if '.' in files:
                    filePath = path + '\\' + files
                    ...
                    if len(idx[0]) != 0:
                        ...
                        if self.hydrogenRange == 'R201':
                            OP = self.dataBrowser(self.df, '201 B', opIdx = 1)
                            PV = self.dataBrowser(self.df, '201 B')
                            PPM = self.dataBrowser(self.df, '201-1', '201 - 1', '201 -1')
                        elif self.hydrogenRange == 'R202':
                            OP = self.dataBrowser(self.df, '202 B', opIdx = 1)
                            PV = self.dataBrowser(self.df, '202 B')
                            PPM = self.dataBrowser(self.df, '201-2', '201 - 2', '201 -2')
                        elif self.hydrogenRange == 'R201NEW' :
                            OP = self.dataBrowser(self.df, 'A      B', formatNew = 'R201NEW', opIdx = 1)
                            PV = self.dataBrowser(self.df, 'A      B', formatNew = 'R201NEW')
                            PPM = self.dataBrowser(self.df, '201-1', '201 - 1', '201 -1', formatNew = 'R201NEW')
                        elif self.hydrogenRange == 'R202NEW' :
                            OP = self.dataBrowser(self.df, 'A      B', formatNew = 'R202NEW', opIdx = 1)
                            PV = self.dataBrowser(self.df, 'A      B', formatNew = 'R202NEW')
                            PPM = self.dataBrowser(self.df, '201-2', '201 - 2', '201 -2', formatNew = 'R202NEW')
##                        elif hydrogenRange == 'R202NEW' :
##                            OP = self.dataBrowser(self.df, 'A      B', opIdx = 1)
##                            PV = self.dataBrowser(self.df, 'A      B')
##                            PPM = self.dataBrowser(self.df, '201-2', '201 - 2', '201 -2')
...
##                    self.n += 3
    ...
    def dataBrowser(self, file: pd.DataFrame, *args, formatNew = None, searchObj = 'FC', opIdx = 0):
        dataBlock = np.array([])
        for i in args:
            IdxKey = np.where(file == i)
            if len(IdxKey[0]) != 0:
                ...
                if formatNew == None:
                    FCrow = IdxKey[0][-1]
                    FCcol = IdxKey[1][-1]
                elif formatNew == 'R201NEW' :
                    FCrow = IdxKey[0][0]
                    FCcol = IdxKey[1][0]
                elif formatNew == 'R202NEW':
                    FCrow = IdxKey[0][-1]
                    FCcol = IdxKey[1][-1]
...
gradeKeyHigh =  gradeKey7402 + gradeKey7702 #+gradeKey5637 + gradeKey5402 +
...
myd = dataExt('FC202B_2019')
myd.hydrogenRange = 'R202'
myd.largeExt(r'D:\Polytama Propindo\Production - Documents\Bulk\Operations\2019\LST-2019\LST~1',gradeKeyHigh)
